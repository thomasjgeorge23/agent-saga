...
class WALProfile:
    ...
    @classmethod
    def fit(cls, records: Sequence[Mapping[str, Any]]) -> "WALProfile":
        ...
        for key, values in numeric.items():
            profile.fields[key] = ("number", min(values), max(values))
        return profile
...
def synthesize(profile: WALProfile, *, sagas: int = 100, seed: Optional[int] = None,
    ...
    """Generate a structurally valid, hash-chained, unmistakably synthetic WAL.

    Deterministic for a given `seed`, so a fixture can be regenerated byte for
    byte and a failing test stays failing.
    """
    if sagas < 1:
    ...
    # Stamp the chain so `agent-saga verify` accepts it: a fixture that fails
